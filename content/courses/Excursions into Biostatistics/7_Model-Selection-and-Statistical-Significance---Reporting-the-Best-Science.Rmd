...
I have prepared some [Lecture Slides](/courses/Excursions-into-Biostatistics/Model-Selection-and-Statistical-Significance---Reporting-the-Best-Science.pdf) for this session. Unfortunately, they are in pdf format, so you will have to download them straight to your PC. For a more mathematical look at these concepts, I cannot recommend enough [Eduardo García Portugués' blog](https://bookdown.org/egarpor/PM-UC3M/lm-ii-modsel.html).
...
```{r}
pred <- mclust::predict.Mclust(H2_PS_mclust) # get predictions
Mclust_PS.roc <- roc(Sparrows_df$Population.Status, # known outcome
                     pred$z[,1] # probability of assigning one out of two outcomes
                     )
plot(Mclust_PS.roc)
auc(Mclust_PS.roc)
```
...
```{r}
Estimates_df <- data.frame(Mixed_boot[["t"]])
