...
w <- warm_list[i]
m_temp <- ulam(m.M3 , chains = 1, iter = 1000+w , warmup = w, start = start)
...
As we can see, there are quite some outliers in the sampling of the cauchy distribution (`b`). Why is that? Because the cauchy distribution has very heavy tails thus making it more likely to jump to a value that is far out there in terms of posterior probability. Note that this also decreases `n_eff`. Unfortunately, I have no idea what that last row in the plot is...
...
$$w∼Binom(θ,n)$$
$$θ∼Unif(0,1)$$
Our Mentropolis algorithm looks like this:
```{r}
set.seed(42)
