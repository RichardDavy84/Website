...
The model including the bed index variables (`m.H1`) shows a slightly better WAIC than the model that omits the bed variable (`m.H2`), and most of the Akaike weight. Judging from this (and the variation in the bed intercepts of model `m.H1`), we can infer that there's a lot of variability between the flower beds which model `m.H1` addresses. Let me visualise this in a plot:
