...
**Answer:**  *1 and 3 are multiple regressions.*  
...
<!-- Model 3 is a tad out there. While it only contains one slope parameter ($\beta$), it does make use of two variables ($x_i$ and $z_i$). It can be rewritten as $μ_i=α+βx_i–βz_i$. Now, the notation is in line with models 2 and 4, and it should be quite obvious that this is a multiple regression as well.  -->
...
Since the combined effect of both slopes is supposed to be positive, the individual slopes must both be positive. 
